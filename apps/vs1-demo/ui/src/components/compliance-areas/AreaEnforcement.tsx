...
export function AreaEnforcement({ slug, selectedCountry }: Props) {
  ...
  // The label centre is clamped inwards: at DE's 9,0 the rank note is wider
  // than the space right of the marker and would run off the card.
  const markerPct = Math.min(Math.max((marker.value / 10) * 100, 15), 78);
  const markerNote =
    marker.rank === null
  ...
  return (
    <div ref={ref} className="flex flex-col gap-10 desktop-s:flex-row desktop-s:items-center desktop-s:gap-14">
      ...
      <div className="min-w-0 flex-1 rounded-xl bg-gradient-stage p-5 sm:p-8 desktop-s:p-10">
        <div className="rounded-xl bg-surface p-6 shadow-[0_34px_80px_-30px_rgba(2,22,17,0.4)] dark:bg-surface-secondary sm:p-8">
          ...
          <div className="relative mt-28 h-28">
            <motion.div
            ...
            {[0, 5, 10].map((n) => (
              <span
                key={n}
                className="absolute top-10 -translate-x-1/2 text-body-4xs font-semibold text-fg-tertiary"
                style={{ left: `${n * 10}%` }}
              >
                {n}
              </span>
            ...
            {groups.map(([value, codes], gi) => {
              ...
              return (
                <div key={value} aria-hidden>
                  ...
                  />
                  <div
                    className="absolute bottom-[6.25rem] flex -translate-x-1/2 flex-col-reverse items-center gap-1.5"
                    style={{ left: `${pct}%` }}
                  >
                    {codes.map((code, si) => {
                      ...
                      return (
                        <motion.span
                          ...
                          className={`w-9 rounded-full border py-0.5 text-center text-body-4xs font-bold tracking-[0.06em] ${
                            own
            ...
            <motion.div
              ...
              animate={inView || reduced ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.35, ease: 'easeOut', delay: markerDelay + 0.1 }}
              className="absolute top-12 -translate-x-1/2 text-center"
              style={{ left: `${markerPct}%` }}
            >
              <span className="block whitespace-nowrap text-body-xs font-bold tabular-nums text-accent-700 dark:text-fg-accent-strong">
                {oneDecimal.format(marker.value)} / 10
              </span>
              <span className="mt-0.5 block whitespace-nowrap text-body-3xs text-fg-tertiary">
                {markerNote}
              </span>
            </motion.div>
          </div>
          ...
          {authorities.length > 0 && (
            <div className="mt-7 flex flex-wrap items-center gap-x-4 gap-y-2.5 border-t border-stroke-subtle pt-5">
              ...
              <div className="flex flex-wrap gap-2">
                {authorities.map((a) => (
                  <span
                    key={a}
                    className="rounded-full border border-stroke px-3.5 py-[0.4375rem] text-body-xs font-semibold text-fg"
                  >
                    {a}
                  </span>
                ))}
