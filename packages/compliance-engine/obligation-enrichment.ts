...
export type PenaltyCeiling =
    ...
    | { kind: 'amount'; value: number; currency: Waehrung; basis: string; asOf: string }
    ...
    | { kind: 'turnover'; percent: number; orAmount?: { value: number; currency: Waehrung }; basis: string; asOf: string }
    ...
    | { kind: 'proportional'; percent: number; of: string; basis: string; asOf: string }
...
export const ObligationEnrichmentMap: EnrichmentMap = {
    ...
    'data-privacy': {
        ...
        US: { source: 'CCPA/CPRA + state privacy acts', penalty: '$2,500–$7,500 per violation', penaltyMaxEur: 50000, due: 'Ongoing', penaltyCeiling: { kind: 'subnational', level: 'Bundesstaaten mit eigenem Datenschutzgesetz (Kalifornien und rund 20 weitere)', basis: 'Cal. Civ. Code § 1798.155 (CCPA/CPRA) und die Parallelgesetze der uebrigen Staaten', note: 'Es gibt kein allgemeines Bundesdatenschutzgesetz; jeder Staat mit eigenem Gesetz setzt eigene Saetze. Die gefuehrten stehen in `states`. Fuer New York fehlt der Zugang zum Gesetzestext, nicht das Gesetz — siehe STAATEN_OHNE_ZUGANG im Waechter.' }, states: {
            ...
            TX: { source: 'Texas Data Privacy and Security Act, Tex. Bus. & Com. Code § 541.155', penaltyCeiling: { kind: 'amount', value: 7500, currency: 'USD', basis: 'Tex. Bus. & Com. Code § 541.155 Buchst. a — 7.500 USD je Verstoss. Anders als Kalifornien und Florida kennt Texas keine Staffelung: EIN Satz, aber erst NACH der Heilungsfrist des § 541.154 (30 Tage ab Mitteilung des Attorney General) oder bei Bruch einer ihm gegenueber abgegebenen schriftlichen Zusage. Durchsetzung allein durch den Attorney General (§ 541.151), ein privates Klagerecht besteht nicht (§ 541.156).', asOf: '2026-09-19' } },
        } },
    ...
    'data-hosting': {
        ...
        US: { source: 'EU-US Data Privacy Framework', penalty: 'Streichung von der Liste; Wegfall der Uebermittlungsgrundlage', penaltyMaxEur: 30000, due: 'Annual', dueDays: 180, penaltyCeiling: { kind: 'none', basis: 'Durchfuehrungsbeschluss (EU) 2023/1795 EG 61 und 72 sowie Anhang I Abschnitt 7 Buchst. a', note: 'Der Rahmen selbst sieht keine Geldbusse vor: die Grundsaetze verlangen in Anhang I Abschnitt 7 nur "hinreichend strenge" Sanktionen, ohne einen Betrag zu nennen. Die Folge eines Verstosses ist die Streichung von der Datenschutzrahmen-Liste nach 30-taegiger Anhoerung und damit der Wegfall der Uebermittlungsgrundlage (EG 72). Eine Geldbusse entsteht erst eine Stufe spaeter: die FTC erwirkt zunaechst eine Anordnung, und erst deren Missachtung kostet Geld (EG 61) — dann nach FTC Act § 5 Buchst. l je Verstoss, bei Luftverkehrsgesellschaften nach 49 U.S.C. § 41712 mit 37.377 USD.' } },
    },
    'corp-registration': {
    ...
    'log-eori': {
        UK: { source: 'UK EORI (HMRC, post-Brexit)', penalty: 'goods held at border; storage costs', penaltyMaxEur: 10000, due: 'One-off', dueDays: 14, penaltyCeiling: { kind: 'none', basis: 'FA 2003 s. 26 i.V.m. SI 2003/3113 Reg. 3 und Anhang', note: 'Der Anhang zaehlt die bussgeldbewehrten Zollpflichten abschliessend auf (Spalte 3: 1.000 oder 2.500 GBP je Zuwiderhandlung). Eine fehlende EORI-Nummer steht nicht darunter. Die Folge ist operativ: die Ware wird nicht abgefertigt, es entstehen Lager- und Standgelder — keine Geldbusse, die man zitieren koennte.' } },
    ...
    'log-intrastat': {
        ...
        IT: { source: 'Intrastat (Agenzia Dogane), modelli INTRA', penalty: 'sanzioni EUR 500 bis 1.000 je Liste', penaltyMaxEur: 1000, due: 'Monthly', dueDays: 25, penaltyCeiling: { kind: 'amount', value: 1000, currency: 'EUR', basis: 'Testo unico delle sanzioni tributarie (D.Lgs. 173/2024) Art. 36 Abs. 7 — je Liste; auf die Haelfte ermaessigt bei Vorlage binnen 30 Tagen nach Aufforderung', asOf: '2026-09-19' } },
        ES: { source: 'Intrastat (AEAT), umbral €400k', penalty: 'multas estadísticas hasta €30.000', penaltyMaxEur: 30050.61, due: 'Monthly', dueDays: 12, penaltyCeiling: { kind: 'amount', value: 30050.61, currency: 'EUR', basis: 'Ley 12/1989 Art. 51 Abs. 1 (infraccion muy grave; 2001 aus Peseten umgestellt, seither nicht angepasst)', asOf: '2026-09-17' } },
