...
Deno.serve(async (req: Request) => {
  ...
  await supabase.from('event_log').insert({ type: 'provider_declined_via_magic_link', payload: { engagementId: id } });

  return new Response(page('Decline request', 'Request Declined', 'You have declined this engagement request. We will seamlessly redirect the client to the next best matched provider.', '#dc2626'), {
    headers: { 'Content-Type': 'text/html' },
    status: 200,
  });
});
