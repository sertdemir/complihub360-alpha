...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === 'GET') {
    const form = `<form method="POST"><button type="submit">Request more information</button></form>`;
    return new Response(page('Request more info', 'Ask the client for more information?', 'The client will be prompted to provide additional details for this request.', '#d4af37', form), {
  ...
  await supabase.from('event_log').insert({ type: 'provider_replied_via_magic_link', payload: { engagementId: id } });

  return new Response(page('Request more info', 'Marked as Replied', 'The system has logged your request for more information. The client will be prompted to provide additional details.', '#0284c7'), {
    headers: { 'Content-Type': 'text/html' },
    status: 200,
  });
});
